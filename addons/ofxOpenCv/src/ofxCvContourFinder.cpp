...
void ofxCvContourFinder::reset() {
    ...
    centerX_of_Blobs.clear();
    centerY_of_Blobs.clear();
...
int ofxCvContourFinder::getSize(){
	return (int)blobs.size();
}

vector<float> ofxCvContourFinder::getCenterX( float x, float y, float w, float h ) {
    for( int i=0; i<(int)blobs.size(); i++ ) {
         centerX_of_Blobs.push_back( (blobs[i].boundingRect.x + blobs[i].boundingRect.width) /2 );
    }
    return centerX_of_Blobs;
}

vector<float> ofxCvContourFinder::getCenterY( float x, float y, float w, float h ) {
    for( int i=0; i<(int)blobs.size(); i++ ) {
        centerY_of_Blobs.push_back( (blobs[i].boundingRect.y + blobs[i].boundingRect.height) /2 );
    }
    return centerY_of_Blobs;
}
//by sena////////
